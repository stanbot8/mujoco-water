...
def main():
    ...
    # 2. Backend
    print("Which physics backend?")
    backend_opts = {"1": "MuJoCo (robotics, RL)", "2": "Chrono (engineering, FSI)", "3": "Standalone (no physics engine)"}
    backend_choice = ask("Pick a backend", backend_opts, "1")
    backend = ["mujoco", "chrono", "standalone"][int(backend_choice) - 1]
    print(f"\n  -> {backend}")
    print()
    ...
    features["python"] = ask_yn("Python bindings (pybind11)?", False)
    print()
