...
static void DrawPanel(WaterEngine& engine, const WaterEngineConfig& cfg,
  ...
  if (ImGui::CollapsingHeader("Simulation", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    float depth = g_z_top + g_z_bottom;
    if (ImGui::SliderFloat("Water depth", &depth, 5.0f, 5000.0f, "%.0f m", ImGuiSliderFlags_Logarithmic)) {
      g_z_top = depth * 0.4f;
      g_z_bottom = depth * 0.6f;
      g_reload_model = true;
    }
  }
  ...
  if (d && bodies && !bodies->empty() &&
    ...
    for (size_t bi = 0; bi < bodies->size(); ++bi) {
      ...
      float buoyancy = 1000.0f * 9.81f * cb.coupling.volume * submersion;
...
int main() {
  ...
  while (viewer.Running()) {
    ...
    if (g_reload_model) {
      ...
      // Restore camera and sync engine config.
      viewer.cam = saved_cam;
      cfg.initial_surface_z = g_z_top * 0.25f;
      engine.Init(cfg);
      PlaceAmoeba();
