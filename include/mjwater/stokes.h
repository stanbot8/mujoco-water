...
struct StokesSolver {
  ...
  Vec3 Velocity(Vec3 pos) const {
    float gx = (pos.x - grid.origin_x) / grid.dx - 0.5f;
    float gy = (pos.y - grid.origin_y) / grid.dx - 0.5f;
    float gz = (pos.z - grid.origin_z) / grid.dx - 0.5f;
    return TrilinearVec3(gx, gy, gz, grid.u, grid.v, grid.w);
  }
  ...
  float Concentration(Vec3 pos) const {
    ...
    float gy = (pos.y - grid.origin_y) / grid.dx - 0.5f;
    float gz = (pos.z - grid.origin_z) / grid.dx - 0.5f;
    return TrilinearScalar(gx, gy, gz, grid.concentration);
  }
  ...
  float Pressure(Vec3 pos) const {
    ...
    float gy = (pos.y - grid.origin_y) / grid.dx - 0.5f;
    float gz = (pos.z - grid.origin_z) / grid.dx - 0.5f;
    return TrilinearScalar(gx, gy, gz, grid.p);
  }
 ...
 private:
    ...
    for (uint32_t z = 1; z + 1 < nz; ++z) {
      for (uint32_t y = 1; y + 1 < ny; ++y) {
        for (uint32_t x = 1; x + 1 < nx; ++x) {
          ...
          auto lap = [&](const std::vector<float>& field) {
            return field[grid.Idx(x+1,y,z)] + field[grid.Idx(x-1,y,z)] +
                   field[grid.Idx(x,y+1,z)] + field[grid.Idx(x,y-1,z)] +
                   field[grid.Idx(x,y,z+1)] + field[grid.Idx(x,y,z-1)] -
                   6.0f * field[c];
          };

          grid.u_tmp[c] = grid.u[c] + alpha * lap(grid.u) + dt * f_per_rho.x;
          grid.v_tmp[c] = grid.v[c] + alpha * lap(grid.v) + dt * f_per_rho.y;
          grid.w_tmp[c] = grid.w[c] + alpha * lap(grid.w) + dt * f_per_rho.z;
        }
      }
    }
    ...
    for (uint32_t z = 1; z + 1 < nz; ++z) {
      for (uint32_t y = 1; y + 1 < ny; ++y) {
        for (uint32_t x = 1; x + 1 < nx; ++x) {
          ...
          grid.div[c] =
            (grid.u[grid.Idx(x+1,y,z)] - grid.u[grid.Idx(x-1,y,z)] +
             grid.v[grid.Idx(x,y+1,z)] - grid.v[grid.Idx(x,y-1,z)] +
             grid.w[grid.Idx(x,y,z+1)] - grid.w[grid.Idx(x,y,z-1)]) * inv_2dx;
        }
      }
    }
  }
    ...
    for (uint32_t z = 1; z + 1 < nz; ++z) {
      for (uint32_t y = 1; y + 1 < ny; ++y) {
        for (uint32_t x = 1; x + 1 < nx; ++x) {
          ...
          float dpdx = (grid.p[grid.Idx(x+1,y,z)] -
                        grid.p[grid.Idx(x-1,y,z)]) * inv_2dx;
          float dpdy = (grid.p[grid.Idx(x,y+1,z)] -
                        grid.p[grid.Idx(x,y-1,z)]) * inv_2dx;
          float dpdz = (grid.p[grid.Idx(x,y,z+1)] -
                        grid.p[grid.Idx(x,y,z-1)]) * inv_2dx;

          grid.u[c] -= dt_over_rho * dpdx;
          grid.v[c] -= dt_over_rho * dpdy;
          grid.w[c] -= dt_over_rho * dpdz;
        }
      }
    }
  }
  ...
  void EnforceBoundaries() {
    ...
    for (uint32_t z = 0; z < nz; ++z) {
      for (uint32_t y = 0; y < ny; ++y) {
        auto zero = [&](uint32_t x) {
          size_t c = grid.Idx(x, y, z);
          grid.u[c] = 0; grid.v[c] = 0; grid.w[c] = 0;
        };
        zero(0);
        zero(nx - 1);
      }
    ...
    for (uint32_t z = 1; z + 1 < nz; ++z) {
      for (uint32_t y = 1; y + 1 < ny; ++y) {
        for (uint32_t x = 1; x + 1 < nx; ++x) {
          ...
          // Diffusion (Laplacian).
          float lap_c =
            grid.concentration[grid.Idx(x+1,y,z)] +
            grid.concentration[grid.Idx(x-1,y,z)] +
            grid.concentration[grid.Idx(x,y+1,z)] +
            grid.concentration[grid.Idx(x,y-1,z)] +
            grid.concentration[grid.Idx(x,y,z+1)] +
            grid.concentration[grid.Idx(x,y,z-1)] -
            6.0f * grid.concentration[c];

          // Advection (upwind).
          float dcdx = (grid.concentration[grid.Idx(x+1,y,z)] -
                        grid.concentration[grid.Idx(x-1,y,z)]) * inv_2dx;
          float dcdy = (grid.concentration[grid.Idx(x,y+1,z)] -
                        grid.concentration[grid.Idx(x,y-1,z)]) * inv_2dx;
          float dcdz = (grid.concentration[grid.Idx(x,y,z+1)] -
                        grid.concentration[grid.Idx(x,y,z-1)]) * inv_2dx;

          float advection = grid.u[c] * dcdx +
                            grid.v[c] * dcdy +
                            grid.w[c] * dcdz;
  ...
  float TrilinearScalar(float gx, float gy, float gz,
    ...
    float c000 = field[grid.Idx(cl(x0,NX),   cl(y0,NY),   cl(z0,NZ))];
    float c100 = field[grid.Idx(cl(x0+1,NX), cl(y0,NY),   cl(z0,NZ))];
    float c010 = field[grid.Idx(cl(x0,NX),   cl(y0+1,NY), cl(z0,NZ))];
    float c110 = field[grid.Idx(cl(x0+1,NX), cl(y0+1,NY), cl(z0,NZ))];
    float c001 = field[grid.Idx(cl(x0,NX),   cl(y0,NY),   cl(z0+1,NZ))];
    float c101 = field[grid.Idx(cl(x0+1,NX), cl(y0,NY),   cl(z0+1,NZ))];
    float c011 = field[grid.Idx(cl(x0,NX),   cl(y0+1,NY), cl(z0+1,NZ))];
    float c111 = field[grid.Idx(cl(x0+1,NX), cl(y0+1,NY), cl(z0+1,NZ))];
  ...
  Vec3 TrilinearVec3(float gx, float gy, float gz,
                      const std::vector<float>& fu,
                      const std::vector<float>& fv,
                      const std::vector<float>& fw) const {
    return {
      TrilinearScalar(gx, gy, gz, fu),
      TrilinearScalar(gx, gy, gz, fv),
      TrilinearScalar(gx, gy, gz, fw)
    };
  }
};
