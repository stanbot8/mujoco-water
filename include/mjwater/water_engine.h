...
struct WaterEngine {
  ...
  // Per-body attachments: lifting surfaces, thrusters, strip config.
  struct BodyAttachments {
    StripConfig strip;                     // strip sampling (length > 0 enables)
    std::vector<LiftingSurface> surfaces;  // fins, rudders, hydrofoils
    std::vector<ThrusterModel> thrusters;  // propellers, jets
  };
  std::vector<BodyAttachments> body_attachments;
  ...
  size_t AddBody(const BodyCoupling& body) {
    ...
    body_attachments.push_back({});
    return bodies.size() - 1;
  }

  // Attach a lifting surface to a body (fin, rudder, hydrofoil).
  void AddSurface(size_t body_idx, const LiftingSurface& s) {
    if (body_idx < body_attachments.size())
      body_attachments[body_idx].surfaces.push_back(s);
  }

  // Attach a thruster to a body (propeller, jet).
  void AddThruster(size_t body_idx, const ThrusterModel& t) {
    if (body_idx < body_attachments.size())
      body_attachments[body_idx].thrusters.push_back(t);
  }

  // Enable strip sampling on a body (distributed Morison loading).
  void SetStrip(size_t body_idx, const StripConfig& s) {
    if (body_idx < body_attachments.size())
      body_attachments[body_idx].strip = s;
  }
  ...
  void Step() {
    ...
    // With defaults (10 * 100 * 10) the theoretical max is 10,000.
    // The guardrail caps total fine steps well below that to keep
    // frame times bounded. When the budget is exhausted, the finest
    // scales are under-resolved for that frame (acceptable for real-time
    // use; reduce substep counts or LOD radii for accuracy).
    ...
    if (sph_active) {
      ...
      for (int s = 0; s < n_sph && total_fine_steps < kMaxFineSteps; ++s) {
        ...
        // 6a. LBM sub-substeps (fixed dt from lattice relaxation time).
        if (lbm_active) {
          ...
          for (int l = 0; l < config.lbm_substeps && total_fine_steps < kMaxFineSteps; ++l) {
            ...
            // 6b. Stokes sub-sub-substeps (CFL from viscous diffusion).
            if (stokes_active) {
  ...
  // body_angular_vel is needed for strip sampling (set to zero if unused).
  CouplingForce ComputeBodyForce(size_t body_idx, Vec3 body_pos,
    ...
    CouplingForce force;
    const auto& attach = body_attachments[body_idx];

    // Morison forces: strip sampling or single-point.
    if (attach.strip.length > 0) {
      auto query_fn = [this](Vec3 p) { return Query(p); };
      force = ComputeStripForces(query_fn, body, attach.strip,
                                  body_pos, body_vel, body_angular_vel,
                                  dt, char_length);
    } else {
      auto fluid = Query(body_pos);
      force = FluidCoupling::ComputeForces(fluid, body, body_vel, dt,
                                            char_length);
    }
    body.prev_fluid_vel = Query(body_pos).velocity;
    ...
    // Lifting surfaces (fins, rudders, hydrofoils).
    for (const auto& surf : attach.surfaces) {
      Vec3 surf_pos = body_pos + surf.offset;
      auto fluid = Query(surf_pos);
      Vec3 surf_vel = body_vel + body_angular_vel.Cross(surf.offset);
      // Submersion from parent body query (approximate).
      float sub = fluid.submerged ? 1.0f : 0.0f;
      auto lift = ComputeLiftDrag(fluid, surf, surf_vel, sub);
      force.force += lift.force;
      force.torque += lift.torque;
    }

    // Thrusters (propellers, jets).
    for (const auto& thr : attach.thrusters) {
      Vec3 thr_pos = body_pos + thr.offset;
      auto fluid = Query(thr_pos);
      auto thrust = ComputeThrustForce(fluid, thr, body_vel);
      force.force += thrust.force;
      force.torque += thrust.torque;
    }
  ...
  struct LayerStats {
    ...
    uint32_t cells;
    float memory_kb;
    float phys_dt_us;  // physics timestep in microseconds (not wall time)
  };
