...
struct WendlandC2 {
  ...
  void Init(float h_) {
    h = h_;
    support = 2.0f * h;
    // 3D normalization for Wendland C2 with support radius H = 2h:
    //   alpha_3d = 21 / (2 * pi * H^3) = 21 / (16 * pi * h^3)
    // The kernel is defined as W(r) = alpha * (1 - r/(2h))^4 * (1 + 2r/h)
    // and integrates to 1 over all space: integral W * 4*pi*r^2 dr = 1.
    // Reference: Wendland (1995); Dehnen & Aly (2012) Table 1.
    norm_3d = 21.0f / (16.0f * kPi * h * h * h);
  }
...
struct SPHParams {
  ...
  float density_floor = 0.9f;       // minimum density as fraction of rest_density (prevents voids)
  float viscosity_epsilon = 0.01f;  // regularization for Monaghan viscosity denominator (prevents 0/0)
...
struct SpatialHash {
  float cell_size = 0;          // set by Init() to kernel support radius
  float inv_cell_size = 0;      // 1.0f / cell_size
  uint32_t table_size = 0;      // set by Init()
 ...
 private:
  ...
  void ComputeDensity() {
    ...
    for (uint32_t a = 0; a < n; ++a) {
      ...
      // Clamp density to prevent unphysical voids. Allowing lower values
      // breaks the weakly compressible assumption. If density drops below
      // this floor, it indicates insufficient neighbors (particle deficiency
      // near free surface or boundaries).
      pa.density = std::max(pa.density, params.rest_density * params.density_floor);
    }
  }
      ...
      hash.ForEachNeighbor(pa.pos, kernel.support, [&](uint32_t b) {
        ...
        if (vr < 0) {
          float mu = params.smoothing_length * vr /
                     (dist * dist + params.viscosity_epsilon * params.smoothing_length * params.smoothing_length);
      ...
      if (params.surface_tension > 0) {
        ...
        hash.ForEachNeighbor(pa.pos, kernel.support, [&](uint32_t b) {
          ...
          float lap_w = kernel.W(dist) * 2.0f *
                        (3.0f / (dist * dist + params.viscosity_epsilon * kernel.h * kernel.h));
          lap_c += vol_b * lap_w;
        });
        float n_len = grad_c.Length();
        // Only apply at the free surface (where |grad(c)| is significant).
        // Interior particles have |grad(c)| ~ 0 (symmetric neighbors).
        // Surface particles have |grad(c)| ~ O(1/h). The threshold is set
        // below the expected surface value to detect the free surface while
        // rejecting interior noise.
        float threshold = 0.75f / kernel.support;
        if (n_len > threshold) {
