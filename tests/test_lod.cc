...
TEST(WaterEngine_ComputeConservation_NoDoubleCounting) {
  // ComputeConservation must subtract SWE mass in the SPH overlap zone
  // to avoid double-counting. Verify total_mass != swe_mass + sph_mass.
  WaterEngine engine;
  WaterEngineConfig cfg;
  cfg.swe_nx = 20;
  cfg.swe_ny = 20;
  ...
  // Activate SPH by focusing at center.
  engine.SetFocus({1, 1, 0.5f});
  for (int i = 0; i < 3; ++i) engine.Step();
  CHECK(engine.sph_active);

  auto diag = engine.ComputeConservation();

  // total_mass should be less than swe_mass + sph_mass because
  // the overlap zone is subtracted from the SWE contribution.
  float naive_sum = diag.swe_mass + diag.sph_mass;
  CHECK(diag.total_mass < naive_sum);

  // total_mass should still be positive and reasonable.
  CHECK(diag.total_mass > 0);

  // The subtracted amount should correspond to SWE cells within the SPH zone.
  float overlap_correction = naive_sum - diag.total_mass;
  CHECK(overlap_correction > 0);
}

TEST(LOD_FluxCorrection_ReducesError) {
  // Verify that CorrectSWESPHInterface adjusts boundary particle masses
  // to account for SWE/SPH flux mismatch.
  ShallowWaterSolver swe;
  swe.Init(20, 20, 0.1f);
  swe.SetSurface(0.5f);
  ...
  SPHSolver sph;
  SPHParams sp;
  sp.smoothing_length = 0.08f;  // wide kernel so many particles are "boundary"
  AABB domain = {{0, 0, 0}, {2, 2, 1}};
  sph.Init(sp, domain);

  AABB sph_zone = {{0.5f, 0.5f, 0}, {1.5f, 1.5f, 1}};
  LODTransition::SWEToSPH(swe, sph, sph_zone, 0.05f);
  float target_mass = swe.TotalVolume() * 0.25f * kWaterDensity;
  LODTransition::CorrectMass_SPH(sph, target_mass);
  CHECK(sph.Count() > 0);

  // Build spatial hash for interpolation.
  sph.hash.Init(sph.kernel.support, sph.Count());
  sph.hash.Build(sph.particles);

  // Create a large fake flux mismatch.
  // CorrectSWESPHInterface computes: swe_mass_flux = net_mass * dt * cell_area * rho.
  // With net_mass=100 (large), dt=0.01, cell_area=0.01, rho=998.2:
  //   swe_mass_flux = 100 * 0.01 * 0.01 * 998.2 = 9.98 kg.
  // Tracker has 0, so mass_err = 9.98 kg (positive = SPH needs more mass).
  SWEInterfaceFlux swe_flux;
  swe_flux.net_mass = 100.0f;
  swe_flux.net_momentum_x = 0;

  SPHFluxTracker tracker;
  tracker.Reset();
  tracker.SetDeadZone(sp.smoothing_length);

  float mass_before = sph.TotalMass();

  LODTransition::CorrectSWESPHInterface(
    swe, sph, swe_flux, tracker, sph_zone, 0.01f);

  float mass_after = sph.TotalMass();

  // Correction should have increased boundary particle masses.
  // The correction is clamped to +/- 5%, so the change is bounded.
  CHECK(mass_after > mass_before);
  // Verify the change is within the expected 5% clamp.
  float max_change = mass_before * 0.05f;
  CHECK(mass_after - mass_before <= max_change + 1e-6f);
}

TEST(SPH_AdaptiveRefine_SplitsNearFocus) {
  // Particles near the focus point with mass > 1.5x nominal should split.
  SPHSolver sph;
  SPHParams sp;
  sp.smoothing_length = 0.02f;
  AABB domain = {{0, 0, 0}, {1, 1, 1}};
  ...
  float nominal_mass = sp.ParticleMass(target_spacing);

  // Add a single heavy particle near the focus.
  SPHParticle p;
  p.pos = {0.5f, 0.5f, 0.5f};
  p.vel = {0.1f, 0, 0};
  p.mass = nominal_mass * 2.0f;  // 2x nominal: should split
  p.density = sp.rest_density;
  sph.particles.push_back(p);

  // Add a normal particle far from focus (should not split).
  SPHParticle p2;
  p2.pos = {0.9f, 0.9f, 0.9f};
  p2.vel = {};
  p2.mass = nominal_mass;
  p2.density = sp.rest_density;
  sph.particles.push_back(p2);

  CHECK(sph.Count() == 2);
  float total_mass_before = sph.TotalMass();

  Vec3 focus = {0.5f, 0.5f, 0.5f};
  sph.AdaptiveRefine(focus, 0.3f, 1.0f, target_spacing);

  // Heavy particle near focus should have split into 2.
  // Far particle should be unchanged.
  CHECK(sph.Count() == 3);

  // Total mass must be conserved (split preserves mass).
  float total_mass_after = sph.TotalMass();
  CHECK_NEAR(total_mass_after, total_mass_before, 1e-8f);

  // Each daughter should have half the original mass.
  // Find the two particles near focus.
  int near_focus = 0;
  for (const auto& part : sph.particles) {
    if ((part.pos - focus).Length() < 0.1f) {
      CHECK_NEAR(part.mass, nominal_mass, 1e-8f);  // 2x / 2 = nominal
      ++near_focus;
    }
  }
  CHECK(near_focus == 2);
...
TEST(SPH_AdaptiveRefine_NoSplitBelowThreshold) {
  // Particles with mass <= 1.5x nominal should not split.
  SPHSolver sph;
  SPHParams sp;
  sp.smoothing_length = 0.02f;
  AABB domain = {{0, 0, 0}, {1, 1, 1}};
  ...
  p.pos = {0.5f, 0.5f, 0.5f};
  p.vel = {};
  p.mass = nominal_mass * 1.4f;  // below 1.5x threshold
  p.density = sp.rest_density;
  sph.particles.push_back(p);

  sph.AdaptiveRefine({0.5f, 0.5f, 0.5f}, 0.3f, 1.0f, target_spacing);
